...
female(valerie). female(barbara). female(donna). female(rachel). female(jane). female(cindy).
...
how_related(X, Y, grandfather) :- grandfather(X, Y).
how_related(X, Y, grandmother) :- grandmother(X, Y).
how_related(X, Y, grandchild) :- grandchild(X, Y).
how_related(X, Y, parent) :- parent(X, Y).
how_related(X, Y, sibling) :- sibling(X, Y).
